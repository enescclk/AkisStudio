(() => {
  ...
  const rawNodes = [
    ...
    ["n6","rect",80,445,120,58,"Hamed\nKhazagachi",{fill:"#ef4444",stroke:"#b91c1c",textColor:"#ffffff",bold:true}],
  ...
  const rawEdges = [
    ...
    ["n7","n8"],["n7","n9"],["n7","n10"],["n11","n8"],["n12","n8"],["n9","n13"],
    ["n15","n16"],["n15","n17"],["n16","n18"],["n16","n19"],["n18","n20"],["n18","n21"],["n19","n22"],
    ["n17","n23"],["n17","n24"],["n17","n25"],["n23","n26"],["n26","n27"],["n26","n28"],["n26","n29"],["n26","n30"],
    ["n24","n31"],["n24","n32"],["n25","n33"],["n25","n34"],["n25","n35"],["n34","n36"],["n35","n37"],
    ["n38","n39"],["n38","n40"],["n39","n41"],["n40","n42"]
  ];
  ...
  A.uid = (prefix) => {
    ...
    return prefix + Date.now().toString(36) + A.runtime.uidCounter.toString(36);
  };

  A.getNode = (id) => A.state.nodes.find((node) => node.id === id);
  A.getEdge = (id) => A.state.edges.find((edge) => edge.id === id);

  A.snapValue = (value) => {
    return A.state.snap ? Math.round(value/10)*10 : value;
  };
  ...
  A.selectedNodeIds = () => {
    const ids=(A.state.selectedNodes||[]).filter((id)=>A.getNode(id));
  ...
  A.selectedNodeObjects = () => {
    return A.selectedNodeIds().map(A.getNode).filter(Boolean);
  };

  A.selectedEdgeIds = () => {
    const ids=(A.state.selectedEdges||[]).filter((id)=>A.getEdge(id));

    if (
      A.state.selectedEdge &&
      !ids.includes(A.state.selectedEdge) &&
      A.getEdge(A.state.selectedEdge)
    ) {
      ids.push(A.state.selectedEdge);
    }

    return [...new Set(ids)];
  };

  A.selectedEdgeObjects = () => {
    return A.selectedEdgeIds().map(A.getEdge).filter(Boolean);
  };

  A.setNodeSelection = (ids) => {
    A.state.selectedNodes=[...new Set(ids)].filter((id)=>A.getNode(id));
  ...
  A.setEdgeSelection = (ids) => {
    A.state.selectedEdges=[...new Set(ids)].filter((id)=>A.getEdge(id));

    A.state.selectedEdge=
      A.state.selectedEdges.length===1
        ? A.state.selectedEdges[0]
        : null;
  };

  A.selectInternalEdges = () => {
    const nodeIds=new Set(A.selectedNodeIds());

    A.state.selectedEdges=A.state.edges
      .filter((edge)=>{
  ...
  A.snapshot = () => JSON.stringify(A.serializable());
  ...
  A.applySnapshot = (input,{markDirty=true}={}) => {
    ...
    A.state.nodes=Array.isArray(data.nodes)
      ? data.nodes.map((node)=>({...A.defaults.node,...node}))
      : [];

    A.state.edges=Array.isArray(data.edges)
      ? data.edges.map((edge)=>({...A.defaults.edge,...edge}))
      : [];
  ...
  A.scheduleSave = (markDirty=true) => {
    ...
    A.runtime.saveTimer=setTimeout(()=>{
      try {
        localStorage.setItem(A.keys.project,A.snapshot());
  ...
  A.setStatus = (message) => {
    ...
    A.setStatus.timer=setTimeout(()=>{
      status.textContent=
        `${A.state.nodes.length} kutu · ${A.state.edges.length} bağlantı`;
    },1800);
  };
